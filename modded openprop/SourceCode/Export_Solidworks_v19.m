...
function [] = Export_Solidworks_v19(filename_SolidWorks,Np,Mp,Z,X3D,Y3D,Z3D,Blade)
...
%section curves
for k = 1:Mp
    filename_SolidWorks_improved = strcat('SectionCurve',num2str(k,'%03.f'),'.csv');
    fid = fopen(filename_SolidWorks_improved,'wt');

    zmean = mean(Z3D(k,:)*1e2);
    
    for j = [1:Np,Np+2:2*Np-1,1] % (2*Np-1 points) does not double print the leading edge
        fprintf(fid,'%f,%f,%f,\n',X3D(k,j,1)*1e2,Y3D(k,j)*1e2,zmean);
    end
